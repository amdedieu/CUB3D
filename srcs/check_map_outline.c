...
/*   Updated: 2021/07/19 13:09:49 by amdedieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_closed(char ** map, int i, int j, int last)
{
	...
	if (map[i][j + 1] == '1' || map[i][j + 1] == ' ') // a droite
		ret++;
	if (last)
		if (map[i + 1][j] == '1' || map[i + 1][j] == ' ') // en dessous
			ret++;
	if (map[i][j - 1] == '1' || map[i][j - 1] == ' ')  // a gauche
		ret++;
	if (map[i - 1][j] == '1' || map[i - 1][j] == ' ') // au dessus
		ret++;
	if (last && ret == 4)
		return 1;
	else if (!last && ret == 3)
