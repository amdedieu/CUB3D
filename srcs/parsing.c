...
/*   Updated: 2021/09/06 12:52:05 by amdedieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((check_double_define(param, ret) == -1) || ret == -1)
...
void	ft_parse_file(t_param *param)
{
	char	*line;
	char	*origin;
	...
	while (get_next_line(param->fd, &line))
	{
		...
		if (interpret_ret(param, check_line(line), line, param->fd) == 1)
			break ;
		free(origin);
	}
}
